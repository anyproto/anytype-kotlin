        ...
        class Date(val binding: ItemCreateFilterDateBinding) : ViewHolder(binding.root) {
            fun bind(item: CreateFilterView.Date) = with(binding) {
                tvDateTitle.text = when (item.type) {
                    ...
                    DVFilterQuickOption.DAYS_AGO -> itemView.resources.getString(R.string.dates_days_ago, "")
                    DVFilterQuickOption.DAYS_AHEAD -> itemView.resources.getString(R.string.dates_days_from, "")
