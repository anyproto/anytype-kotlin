...
class RelationOptionsSubscriptionContainer(
    private val repo: BlockRepository,
    private val channel: SubscriptionEventChannel,
    private val store: StoreOfRelationOptions,
    ...
    fun observe(params: Params): Flow<Index> {
        return flow {
            ...
            store.clear()
            store.merge(
                options = initial.results.map { ObjectWrapper.Option(it.map) }
            )
            val sub = Index(
            ...
            logger.logException(it, "Error in relation options container")
        }.flowOn(
    ...
    companion object {
        const val SUBSCRIPTION_ID = "relation-options-store-subscription"
    }
}
