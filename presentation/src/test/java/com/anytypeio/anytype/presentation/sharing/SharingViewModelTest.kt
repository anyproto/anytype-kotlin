...
class SharingViewModelTest {
    ...
    @Test
    fun `onSendClicked with Mixed content with media should upload files only`() = runTest {
        // Given
        stubAwaitAccountStart()
        stubSpaceViewSubscription(listOf(testDataSpace))
        stubPermissions(mapOf(testDataSpace.id to SpaceMemberPermissions.OWNER))
        ...
        vm.onSharedDataReceived(
            SharedContent.Mixed(
                ...
                url = null,
                mediaUris = listOf("content://test/image.jpg")
            )
        )
        advanceUntilIdle()
        ...
        // Then - should upload files, text should be ignored
        verify(uploadFile).async(any())
        verify(createPrefilledNote, never()).async(any())
    }

    @Test
    fun `onSendClicked with Mixed content without media should create Note`() = runTest {
        // Given
        stubAwaitAccountStart()
        stubSpaceViewSubscription(listOf(testDataSpace))
        stubPermissions(mapOf(testDataSpace.id to SpaceMemberPermissions.OWNER))
        ...
        vm.onSharedDataReceived(
            SharedContent.Mixed(
                text = "Text content without media",
                url = null,
                mediaUris = emptyList()
            )
        )
        ...
        // Then - should create Note since no media
        verify(createPrefilledNote).async(any())
        verify(uploadFile, never()).async(any())
    }
